 ...
 *** from B904SPEGAIN.txt, crates a text file, namely B904SPEInfo.txt                     ***
 ***                                                                                      ***
 ***                                                                                      ***
 ********************************************************************************************/
...
void HFB904SPEInfo() {
  ...
  ofstream out;
  out.open("/txt/B904SPEInfo.txt",ios::out);

  ifstream ins;
  ins.open("/txt/B904SPEGAIN.txt",ios::in);


  for (int no=94; no<134; no++) {

    sprintf(conf,"/txt/Configurations/Conf%d.txt",no);
    cout<<"------"<<conf<<"-----"<<endl;

    ifstream inp;
    inp.open("/txt/ledconf.txt");
	...
	runnumber<<runno<<"_SPENtuples.root";
	TString dir = gSystem->UnixPathName(gInterpreter->GetCurrentMacroName());
	dir.ReplaceAll("./HFB904SPEInfo.C","HFSPE/SPENtuples/");
	TString rootfile = dir+runnumber.str().c_str();
	runnumber.str("");

	TFile *f = new TFile(rootfile);

	TTree *tree = (TTree*)f->Get("HFSPEInfo");
	...
	if(no==98 || no==103 || no==109 || no==114) {
	  ...
	  for(int j=0; j<3; j++) {
	    ...
	    in>>Base[j];
	    sprintf(baseno,"/txt/Baseboards/Baseboard%d.txt",Base[j]);
