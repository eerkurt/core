     ...
     /*******************************************************************************************
      ****** PROGRAM : AllHFSPEkdistFromText.C **************                                  ***
      *******************************************************                                  ***
      ***                                                                                      ***
...
void AllHFSPEkdistFromText() {

  Int_t runno,qie,db;
  Double_t k;

  stringstream pathname1;
  pathname1<<"AllSPEkdist.root";
  TString dir1 = gSystem->UnixPathName(gInterpreter->GetCurrentMacroName());
  dir1.ReplaceAll("./AllSPEkdistFromText.C","SPENtuples/");
  TString fullpathname1 = dir1+pathname1.str().c_str();
  TFile *f1 = new TFile(fullpathname1,"RECREATE");

  TCanvas *c1 = new TCanvas("k dist","All SPE k distribution",600,800);
  TH1D *allkdist = new TH1D("k dist","All SPE k distribution",1728,0,2);

  TF1 *fit = new TF1("fit1","gaus",0,2);

  ifstream in;
  in.open("/Users/core/Desktop/!!!THESIS!!!/PMT ANALYSIS /PMTB904/PMTTESTANALYSIS/txt/B904/spe_runlist.txt",ios::in);
  while(in>>runno) {
 
  stringstream pathname;
  pathname<<runno<<"_kdist.txt";
  TString dir = gSystem->UnixPathName(gInterpreter->GetCurrentMacroName());
  dir.ReplaceAll("./AllSPEkdistFromText.C","SPENtuples/");
